...
import {
  Shipment, Driver, Bid, WalletTransaction, ChatMessage, Notification,
  mockShipments, mockDrivers, mockBids, mockWalletTransactions,
  mockDriverTransactions, mockNotifications, AppUser,
...
interface AppState {
  ...
  getBrokerShipments: (brokerId: string) => Shipment[];
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    AsyncStorage.getItem('jammal_shipments_v2').then((data) => {
      if (data) {
        try {
          const parsed = JSON.parse(data);
          if (parsed.length > 0) setShipments(parsed);
        } catch {}
      }
    });
  }, []);
  ...
  useEffect(() => {
    AsyncStorage.setItem('jammal_shipments_v2', JSON.stringify(shipments));
  }, [shipments]);
    ...
    setShipments((prev) =>
      prev.map((s) => {
        ...
        if (status === 'delivered') {
          ...
          updated.trackingProgress = 1;
        }
        if (status === 'en_route') {
          updated.trackingProgress = 0.3;
        }
        if (status === 'pickup') {
          updated.trackingProgress = 0.1;
        }
        return updated;
      })
    );
  ...
  const acceptBid = (shipmentId: string, bidId: string) => {
      ...
      prev.map((s) =>
        s.id === shipmentId
          ? {
              ...
              driverNameAr: bid.driver.nameAr,
              driverRating: bid.driver.rating,
    ...
    const newTx: WalletTransaction = {
      ...
      descriptionAr: `شحنة ${shipmentId}`,
      date: new Date().toISOString().split('T')[0],
      status: 'pending',
      shipmentId,
    };
    setWalletTransactions((prev) => [newTx, ...prev]);
    setWalletBalance((prev) => prev - bid.amount);
  };

  const getShipmentBids = (shipmentId: string) => bids.filter((b) => b.shipmentId === shipmentId);

  const getActiveShipments = () =>
    shipments.filter((s) => !['delivered', 'disputed', 'draft'].includes(s.status));

  const getShipmentById = (id: string) => shipments.find((s) => s.id === id);

  const getDriverShipments = (driverId: string) =>
    shipments.filter((s) => s.driverId === driverId || s.driverName);

  const getBrokerShipments = (brokerId: string) =>
    shipments.filter((s) => s.brokerId === brokerId);
  ...
  const sendChatMessage = (shipmentId: string, senderId: string, senderName: string, text: string) => {
    ...
    setTimeout(() => {
      const reply: ChatMessage = {
        ...
        senderId: 'auto-reply',
        senderName: 'السائق',
        text: 'تم الاستلام، سأكون هناك قريباً إن شاء الله',
        timestamp: new Date().toISOString(),
        isMe: false,
      };
  ...
  const getChatMessages = (shipmentId: string) =>
    chatMessages.filter((m) => m.shipmentId === shipmentId);
  ...
  const processPayment = (shipmentId: string, method: string): boolean => {
    setShipments((prev) =>
      prev.map((s) =>
        s.id === shipmentId ? { ...s, paymentStatus: 'authorized', paymentMethod: method } : s
      )
    );
    return true;
  };
  ...
  const submitBid = (shipmentId: string, driver: Driver, amount: number, message: string) => {
    ...
    setShipments((prev) =>
      prev.map((s) => (s.id === shipmentId ? { ...s, bidsCount: s.bidsCount + 1 } : s))
    );
  };
  ...
  return (
    <AppContext.Provider
      value={{
        shipments, drivers, bids, walletTransactions, driverTransactions,
        notifications, chatMessages, walletBalance, driverWalletBalance,
        driverOnline, setDriverOnline, addShipment, updateShipmentStatus,
        acceptBid, getShipmentBids, getActiveShipments, getShipmentById,
        getDriverShipments, getBrokerShipments, sendChatMessage,
        getChatMessages, markNotificationRead, getUnreadNotifCount,
        processPayment, submitBid,
      }}
    >
      {children}
    </AppContext.Provider>
