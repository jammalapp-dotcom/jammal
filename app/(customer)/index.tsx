      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: insets.bottom + 16 }} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View entering={FadeInDown.delay(100).duration(500)} style={styles.header}>
          <View>
            ...
            <Text style={styles.userName}>{user?.name || 'عميل'}</Text>
          </View>
          <View style={styles.headerRight}>
            <Pressable style={styles.notifBtn} onPress={() => router.push('/notifications')}>
              ...
              {getUnreadNotifCount() > 0 && (
                <View style={styles.notifBadge}><Text style={styles.notifBadgeText}>{getUnreadNotifCount()}</Text></View>
              )}
            </Pressable>
          ...
          <LinearGradient colors={[theme.primary, theme.primaryLight]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.balanceCard}>
            <View style={styles.balanceTop}>
              <View>
                ...
                <Text style={styles.balanceValue}>{walletBalance.toLocaleString()} <Text style={styles.balanceCurrency}>ر.س</Text></Text>
              </View>
              <Pressable style={styles.topUpBtn} onPress={() => showAlert('شحن المحفظة', 'خاصية شحن المحفظة ستكون متاحة قريباً عبر مدى، Apple Pay، و STC Pay')}>
        ...
        <Animated.View entering={FadeInDown.delay(400).duration(500)}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>الشحنات النشطة</Text>
            <Pressable onPress={() => router.push('/(customer)/shipments')}><Text style={styles.seeAll}>عرض الكل</Text></Pressable>
          </View>
                ...
                return (
                  <Pressable key={s.id} style={styles.shipCard} onPress={() => router.push(`/shipment/${s.id}`)}>
                    <View style={styles.shipCardTop}>
                      <Text style={styles.shipId}>{s.id}</Text>
                      <View style={[styles.badge, { backgroundColor: st.color + '15' }]}><View style={[styles.badgeDot, { backgroundColor: st.color }]} /><Text style={[styles.badgeText, { color: st.color }]}>{st.labelAr}</Text></View>
                    </View>
                    ...
                    {s.trackingProgress !== undefined && (
                      <View style={styles.trackRow}>
                        <View style={styles.trackBg}><View style={[styles.trackFill, { width: `${s.trackingProgress * 100}%` }]} /></View>
                        <Text style={styles.trackPct}>{Math.round(s.trackingProgress * 100)}٪</Text>
                      </View>
                    )}
                    <View style={styles.shipCardBottom}>
                      <Text style={styles.cargoType}>{s.cargoCategoryAr}</Text>
        ...
        {/* Tracking Banner */}
        {activeShipments.some((s) => s.status === 'en_route') && (
          <Animated.View entering={FadeInDown.delay(500).duration(500)}>
            <Pressable style={styles.trackBanner} onPress={() => { const s = activeShipments.find((x) => x.status === 'en_route'); if (s) router.push(`/shipment/${s.id}`); }}>
              ...
              <View style={styles.trackContent}>
                <View><Text style={styles.trackLabel}>شحنة في الطريق</Text><Text style={styles.trackRoute}>{activeShipments.find((s) => s.status === 'en_route')?.pickupCity} → {activeShipments.find((s) => s.status === 'en_route')?.deliveryCity}</Text></View>
                <View style={styles.trackBtn}><MaterialIcons name="my-location" size={20} color="#FFF" /><Text style={styles.trackBtnText}>تتبع</Text></View>
              </View>
            </Pressable>
          </Animated.View>
        )}
...
const styles = StyleSheet.create({
  ...
  trackRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 },
  trackBg: { flex: 1, height: 6, backgroundColor: theme.backgroundSecondary, borderRadius: 3, overflow: 'hidden' },
  trackFill: { height: 6, backgroundColor: theme.accent, borderRadius: 3 },
  trackPct: { fontSize: 12, fontWeight: '600', color: theme.accent },
  ...
  trackBanner: { marginHorizontal: 16, marginTop: 16, height: 120, borderRadius: 16, overflow: 'hidden' },
  trackOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(27,42,74,0.75)' },
  trackContent: { flex: 1, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20 },
  trackLabel: { fontSize: 12, color: 'rgba(255,255,255,0.7)' },
  trackRoute: { fontSize: 18, fontWeight: '700', color: '#FFF', marginTop: 4 },
  trackBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: theme.accent, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20 },
