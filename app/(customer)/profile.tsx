...
export default function CustomerProfileScreen() {
  ...
  const { walletBalance, walletTransactions, shipments } = useApp();
  ...
  const settings = [
    ...
    { icon: 'credit-card', label: 'طرق الدفع', desc: 'مدى، Apple Pay، STC Pay', onPress: () => showAlert('طرق الدفع', 'طرق الدفع المتاحة:\n\n• مدى (**** ٤٥٦٧)\n• Apple Pay\n• STC Pay\n• بطاقة ائتمان') },
    { icon: 'notifications', label: 'الإشعارات', desc: 'إدارة الإشعارات', onPress: () => router.push('/notifications') },
    { icon: 'language', label: 'اللغة', desc: 'العربية', onPress: () => showAlert('اللغة', 'اللغة الحالية: العربية\n\nدعم اللغة الإنجليزية قريباً') },
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: insets.bottom + 16 }} showsVerticalScrollIndicator={false}>
        <Animated.View entering={FadeInDown.delay(100).duration(500)}>
          <LinearGradient colors={[theme.primary, theme.primaryLight]} style={styles.profileHeader}>
            ...
            <View style={styles.statsRow}>
              <View style={styles.stat}><Text style={styles.statVal}>{user?.rating || 0}</Text><Text style={styles.statLbl}>التقييم</Text></View>
              <View style={styles.statDiv} />
              <View style={styles.stat}><Text style={styles.statVal}>{completed}</Text><Text style={styles.statLbl}>مكتملة</Text></View>
              <View style={styles.statDiv} />
              <View style={styles.stat}><Text style={styles.statVal}>{active}</Text><Text style={styles.statLbl}>نشطة</Text></View>
            </View>
          </LinearGradient>
        </Animated.View>
        ...
        <Animated.View entering={FadeInDown.delay(200).duration(500)}>
          <View style={styles.walletCard}>
            <View style={styles.walletRow}>
              <View><Text style={styles.walletLabel}>رصيد المحفظة</Text><Text style={styles.walletVal}>{walletBalance.toLocaleString()} <Text style={styles.walletCur}>ر.س</Text></Text></View>
              <Pressable style={styles.walletBtn} onPress={() => showAlert('شحن المحفظة', 'خاصية شحن المحفظة ستكون متاحة قريباً عبر مدى، Apple Pay، و STC Pay')}><MaterialIcons name="add" size={20} color={theme.accent} /><Text style={styles.walletBtnText}>شحن</Text></Pressable>
            </View>
            <View style={styles.divider} />
            <Text style={styles.txTitle}>آخر المعاملات</Text>
            {walletTransactions.slice(0, 4).map((tx) => (
              <View key={tx.id} style={styles.txRow}>
                <View style={[styles.txIcon, { backgroundColor: tx.type === 'refund' ? theme.successLight : theme.errorLight }]}>
                  <MaterialIcons name={tx.type === 'refund' ? 'keyboard-return' : 'payment'} size={18} color={tx.type === 'refund' ? theme.success : theme.error} />
                </View>
                <View style={{ flex: 1 }}><Text style={styles.txDesc}>{tx.descriptionAr}</Text><Text style={styles.txDate}>{tx.date}</Text></View>
                <Text style={[styles.txAmt, { color: tx.amount > 0 ? theme.success : theme.textPrimary }]}>{tx.amount > 0 ? '+' : ''}{tx.amount.toLocaleString()} ر.س</Text>
              </View>
            ))}
          </View>
        </Animated.View>
        ...
        <Animated.View entering={FadeInDown.delay(300).duration(500)}>
          <View style={styles.settingsCard}>
            {settings.map((s, i) => (
              <Pressable key={s.label} style={[styles.settingsRow, i < settings.length - 1 && styles.settingsRowBorder]} onPress={s.onPress}>
                <View style={styles.settingsIcon}><MaterialIcons name={s.icon as any} size={22} color={theme.primary} /></View>
                <View style={{ flex: 1 }}><Text style={styles.settingsLabel}>{s.label}</Text><Text style={styles.settingsDesc}>{s.desc}</Text></View>
...
const styles = StyleSheet.create({
  ...
  profileHeader: { paddingHorizontal: 16, paddingTop: 24, paddingBottom: 28, alignItems: 'center' },
  ...
  profileCompany: { fontSize: 14, color: 'rgba(255,255,255,0.7)', marginTop: 4 },
  ...
  walletLabel: { fontSize: 12, color: theme.textSecondary },
  walletVal: { fontSize: 24, fontWeight: '700', color: theme.textPrimary, marginTop: 2 },
  walletCur: { fontSize: 14, fontWeight: '500', color: theme.textSecondary },
  ...
  walletBtnText: { fontSize: 13, fontWeight: '600', color: theme.accent },
  divider: { height: 1, backgroundColor: theme.borderLight, marginVertical: 14 },
  txTitle: { fontSize: 14, fontWeight: '600', color: theme.textSecondary, marginBottom: 10 },
  ...
  settingsRowBorder: { borderBottomWidth: 1, borderBottomColor: theme.borderLight },
  settingsIcon: { width: 40, height: 40, borderRadius: 10, backgroundColor: theme.primary + '08', alignItems: 'center', justifyContent: 'center' },
  settingsLabel: { fontSize: 15, fontWeight: '600', color: theme.textPrimary },
  settingsDesc: { fontSize: 12, color: theme.textSecondary, marginTop: 2 },
